...
@Component
public class ReadEmployeeSegmentDepartmentQueryHandler implements RequestHandler<ReadEmployeeSegmentDepartmentQuery, Object> {
    ...
    @Override
    public Object handle(ReadEmployeeSegmentDepartmentQuery query) {
        ...
        var result = repository.getEmployeeCountBySegmentSalaryGroupByDepartment(segment.getMinSalary(), segment.getMaxSalary());
        if (!result.isPresent()) {
            return null;
        }
        return result.get();
